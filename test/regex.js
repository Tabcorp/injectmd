...
test('match files with a loose regex', function (t) {
  t.plan(2)
  ...
  const tasks = [
    ...
    function readFile (_, next) {
      fs.readFile(path.join(__dirname, 'fixtures/file-regex.md'), 'utf8', next)
    },
    ...
    function runTask (next) {
      ...
      child.on('close', function () {
        const expath = path.join(__dirname, 'fixtures/expected-regex.md')
